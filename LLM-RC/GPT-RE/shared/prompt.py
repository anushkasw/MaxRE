...
from shared.const import tacred_def_choice, tacred_def_choice_na, task_def_choice, choice_def_na, task_def_choice_na, choice_def
...
class instance:
    def __init__(self, tmp_dict):
        ...
        self.head = " ".join(tmp_dict["token"][sub_head:sub_tail])
        self.head_type = tmp_dict["stanford_ner"][sub_head]
        self.tail = " ".join(tmp_dict["token"][obj_head:obj_tail])
        self.tail_type = tmp_dict["stanford_ner"][obj_head]
    ...
    def get_relation(self, tmp_dict):
        # if tmp_dict["relation"] in ['no_relation', 'Other', 'NA', 'nan']:
        #     return "NONE"
        # else:
        return tmp_dict["relation"]
...
def generate_select_auto_prompt(tmp_dict, example_prompt, relation_dict, no_na, reasoning, args):
    '''
    This function creates the final prompt for a test sample by concatenating the general instructions with context and query
    :param tmp_dict:
    ...
    if True:
        ...
        if args.task not in ['dummy_tacred', 'tacred', 'retacred', 'GIDS', 'semeval_nodir', 'kbp37_nodir']:
        ...
        # if no_na:
        #     prompt = task_def_choice + choice_def_na + example_prompt + query
        ...
        # return prompt, entity1, entityntity2
...
def generate_self_error_prompt(tmp_dict, example_prompt, relation_dict, no_na, reasoning, args):
    ...
    if True:
        ...
        if args.task in ["tacred", "tacrev", "dummy_tacred", "retacred"]:
            if no_na:
